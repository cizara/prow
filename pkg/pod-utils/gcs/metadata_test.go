...
func TestWriterOptionsFromFileName(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedAttrs: io.WriterOptions{
				ContentType: utilpointer.To("text/plain; charset=utf-8"),
			},
		},
		{
			name:             "txt.gz",
			...
			expectedFileName: "build-log.txt",
			expectedAttrs: io.WriterOptions{
				ContentEncoding: utilpointer.To("gzip"),
				ContentType:     utilpointer.To("text/plain; charset=utf-8"),
			},
		},
		{
		...
		},
		{
			...
			expectedFileName: "gz",
			expectedAttrs: io.WriterOptions{
				ContentType: utilpointer.To("application/gzip"),
			},
		},
		{
			name:             "gz",
		...
		},
		{
			...
			expectedAttrs: io.WriterOptions{
				ContentType: utilpointer.To("application/json"),
			},
		},
		{
			name:             "json.gz",
